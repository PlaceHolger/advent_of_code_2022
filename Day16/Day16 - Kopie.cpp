...
constexpr const char* const inputData[] =
{
...
constexpr int numDataEntries = std::size(inputData);
...
CDataRoom datarooms[numDataEntries];
...
struct CRoute
{
	...
	int targetId;
	vector<CRoom> path;
	int GetCost() { return (int)path.size() + 1; }
	//todo: calc benefits on the way (open valves...)
};
...
struct CRoom
{
	...
	int flowrate;
	bool open = true;
	vector<CRoute> connections;
};
vector<CRoom> rooms;

CRoom& GetRoom(int id)
{
	for (auto& value : rooms)
	{
		if (value.id == id)
			return value;
	}
...
int GetRoomId(const char name[4])
{
	return name[0] * 1000 + name[1];
}
...
int main()
{
	...
	{
		const int result = sscanf(inputData[i], "Valve %2s has flow rate=%i; tunnels lead to valves %2s, %2s, %2s, %2s, %2s", datarooms[i].name, &datarooms[i].flowrate, datarooms[i].neighbors[0], datarooms[i].neighbors[1], datarooms[i].neighbors[2], datarooms[i].neighbors[3], datarooms[i].neighbors[4]);
		if (result < 3)
			std::cerr << "Data not fine!";
	}

	for (auto& dataroom : datarooms)
	{
		rooms.emplace_back();
		CRoom& newRoom = rooms.back();
		newRoom.id = GetRoomId(dataroom.name);
		newRoom.flowrate = dataroom.flowrate;
	}
	...
	for (int i = 0; i < numDataEntries; ++i)
	{
		for (int j = 0; j < 5; ++j)
		{
			if(datarooms[i].neighbors[j][0] != 0)
			{
				CRoute newRoute{ rooms[i].id, GetRoomId(datarooms[i].neighbors[j]) };
			}
		}
		//rooms[i].connections.push_back(newRoute);
	}	
}